...
def clean_movie_title(filename):
    """
    Universal Title Parser (Token Stream Boundary Engine):
    - Strips leading channel usernames, branding & bracket tags
    - Converts dots/underscores to spaces for 100% boundary accuracy
    - Accurately detects and cuts at S01E40, S01E661, Season 1, Episode 23, Year, Quality
    - Guaranteed pure Title output for accurate TMDb Poster & Auto-merge
    """
    # 1. Remove extension
    name = re.sub(r"\.(mkv|mp4|avi|webm|zip|rar)$", "", str(filename), flags=re.IGNORECASE)
    
    # 2. Strip starting bracket tags e.g. [@MZ_Botz], [Channel]
    name = re.sub(r"^\s*(\[.*?\]|\(.*?\))\s*", "", name)

    # 3. Strip starting raw Channel Usernames/Tags e.g. '@Raje Movies Devta' -> 'Devta'
    name = re.sub(r"(?i)^\s*@\w+(\s+(movies|cinema|films|series|hub|channel|official|off|tv|network|media))?\s*", "", name)

    # 4. Pre-normalize dots, underscores, dashes to spaces
    ...
    # 6. Universal Season & Episode Detection (Combined + Standalone)
    # Pattern 1: S01E40 / S1E661 / S01 EP40
    se_comb_match = re.search(r"(?i)\bS(\d{1,2})\s*(?:E|EP)\s*\d{1,4}\b", name)
    # Pattern 2: Season 1 / S01 / Season 01
    s_match = re.search(r"(?i)\b(?:season\s*(\d{1,2})|S(\d{1,2}))\b", name)
    # Pattern 3: Episode 661 / Ep 40 / E40 standalone
    ...
    # 7. Quality, Codec, Audio & Tech boundaries
    ...
    # 8. Hard cut strictly at the earliest boundary
    first_cut = min(cut_positions)
    main_title = name[:first_cut]

    # 9. Final trim & cleanup
...
def find_similar_key(new_key):
    """Deterministic merge matching - combines all episodes/qualities of the same movie/season"""
...
async def media(bot, message):
    ...
    try:
        ...
        async with MERGE_LOCK:
            ...
            # -------------------------------------------------------------
            # CASE 2: New Post (First File or New Movie/Season)
            # -------------------------------------------------------------
            imdb_info = None
            try:
