...
def clean_movie_title(filename):
    ...
    # 1. Tags aur brackets pehle hi clean karein
    name = re.sub(r"\[.*?\]|\(.*?\)", " ", name)
    ...
    # 3. Detect Season & Episode (e.g. S01E01, Season 1, S1)
...
async def media(bot, message):
    ...
    try:
        ...
        async with MERGE_LOCK:
            ...
            # -------------------------------------------------------------
            # CASE 1: Merge Mode (Episodes merge into existing post)
            # -------------------------------------------------------------
            if matched_key:
            ...
            imdb_info = None
            try:
                # YEAR AUR FILE PASS KIYA GAYA HAI WRONG POSTER ROKNE KE LIYE
