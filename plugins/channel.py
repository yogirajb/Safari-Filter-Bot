...
def extract_movie_details(filename):
    """Accurately extracts clean movie/series title, season, year and merge key"""
    # Remove file extension
    name = re.sub(r"\.(mkv|mp4|avi|webm|zip|rar)$", "", str(filename), flags=re.IGNORECASE)
    name = re.sub(r"[\._\-\+]", " ", name)
    
    # 1. Detect Season (e.g. S01, S1, Season 1, Season 02)
    season_match = re.search(r"(?i)\b(s\d+|season\s*\d+)\b", name)
    season_tag = season_match.group(0).upper().replace(" ", "") if season_match else None
    
    # 2. Detect Year (1900-2099)
    year_match = re.search(r"\b(19\d\d|20\d\d)\b", name)
    year = year_match.group(1) if year_match else None
    
    # Cut off everything after season or year
    cut_pos = len(name)
    if season_match:
    ...
    # Remove all extra junk tags, qualities, rip types, codecs, and telegram handles
    junk_patterns = (
    ...
    # Series: 'mirzapur s01' (ताकि S01 के सारे एपिसोड्स/क्वालिटीज़ एक साथ मर्ज हों)
    # Movie: 'ayogya 2'
    merge_key = f"{clean_title.lower()} {season_tag.lower()}" if season_tag else clean_title.lower()
    
    return clean_title, year, season_tag, merge_key
...
async def media(bot, message):
    ...
    try:
        ...
        clean_title, extracted_year, season_tag, merge_key = extract_movie_details(raw_name)
        ...
        # -------------------------------------------------------------
        # CASE 1: अगर इस मूवी/सीरीज़ सीज़न की पोस्ट पहले जा चुकी है, तो MERGE (EDIT) करें
        # -------------------------------------------------------------
        ...
        display_title = f"{clean_title.title()} {season_tag}" if season_tag else clean_title.title()

        if imdb_info:
            title = imdb_info.get('title', display_title)
            if season_tag and season_tag not in title:
        ...
        # Save in cache for merging future qualities / episodes
