...
def clean_movie_title(filename):
    ...
    - Fixes numeric title bug (e.g. 1917)
    - Strips branding & token boundaries cleanly
    """
    ...
    # Smart Year Match: Agar title hi number ho (jaise "1917 2019..."), toh pehle word ko cut mat karo
    all_years = list(re.finditer(r"\b(19\d\d|20\d\d)\b", name))
    extracted_year = None
    if all_years:
        if all_years[0].start() == 0 and len(all_years) > 1:
            # 1917 movie case: pehla saal nahi, dusra saal release year hai
...
async def media(bot, message):
    ...
    try:
        ...
        # -----------------------------------------------------------------
        ep_tag = "_".join(str(e) for e in sorted(list(new_eps))) if new_eps else ""
        current_merge_key = f"{clean_title.lower()}_{season_tag.lower()}_{ep_tag}_{extracted_year or ''}".strip()
        ...
        async with MERGE_LOCK:
            ...
            # -------------------------------------------------------------
            # CASE 1: Post Exists -> Auto-edit with Qualities/Episodes
            # -------------------------------------------------------------
            if matched_key:
            ...
            imdb_info = None
            try:
                # Agar series/season tag hai toh search year pass mat karo (taaki 2026 conflict na ho)
                search_yr = None if season_tag else extracted_year
                imdb_info = await get_poster(clean_title, year=search_yr, file=raw_name)
            ...
            # TMDb se aane wala clean title use karein
            if imdb_info and imdb_info.get("title"):
                title = imdb_info.get("title")
                # Agar title me season tag already nahi hai aur season_tag available hai, tabhi jodein
