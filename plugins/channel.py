...
async def media(bot, message):
    ...
    success, file_id = await save_file(media)[span_1](start_span)[span_1](end_span)
    
    if not success or not file_id:
        return

    file_details = await get_file_details(file_id)[span_2](start_span)[span_2](end_span)
    if file_details:
        file_id = file_details[0]['file_id'][span_3](start_span)[span_3](end_span)
    ...
    try:
        ...
        size_text = get_size(media.file_size)[span_4](start_span)[span_4](end_span)
        bot_uname = temp.U_NAME or (await bot.get_me()).username[span_5](start_span)[span_5](end_span)
        channel_ref = CHANNELS[0] if isinstance(CHANNELS, list) and CHANNELS else "db[span_6](start_span)"[span_6](end_span)
        file_url = f"https://t.me/{bot_uname}?start=files_{channel_ref}_{file_id}[span_7](start_span)"[span_7](end_span)
        
        new_file_entry = (file_url, file_name_display, size_text)

        # Detect Languages
        combined_text = f"{raw_name} {media.caption or ''}[span_8](start_span)"[span_8](end_span)
        ...
        target_channels = POST_CHANNELS if isinstance(POST_CHANNELS, list) else [POST_CHANNELS][span_9](start_span)[span_9](end_span)
        ...
        async with MERGE_LOCK:
            ...
            imdb_info = None
            try:
                imdb_info = await get_poster(clean_title)[span_10](start_span)[span_10](end_span)
            except Exception as e:
            ...
            if imdb_info:
                title = imdb_info.get('title', display_title)[span_11](start_span)[span_11](end_span)
                genres = imdb_info.get('genres', 'Drama, Action')[span_12](start_span)[span_12](end_span)
                year = imdb_info.get('year', extracted_year or '2024')[span_13](start_span)[span_13](end_span)
                rating = imdb_info.get('rating', 'N/A')[span_14](start_span)[span_14](end_span)
                poster_url = imdb_info.get('poster', None)[span_15](start_span)[span_15](end_span)
            else:
                title = display_title
                genres = "Drama, Action"
