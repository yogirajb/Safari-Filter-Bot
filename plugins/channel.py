...
async def media(bot, message):
    ...
    try:
        ...
        async with MERGE_LOCK:
            ...
            imdb_info = None
            try:
                # Serials ke liye search_yr None rakhein taaki 2026/date conflict na ho
                search_yr = None if season_tag else extracted_year
                # Yahan raw_name nahi, clean_title bhejein taaki episode subtitle search ko kharab na kare
                imdb_info = await get_poster(clean_title, year=search_yr, file=clean_title)
                if not imdb_info and search_yr:
                    imdb_info = await get_poster(clean_title, year=None, file=clean_title)
            except Exception as e:
