...
def clean_movie_title(filename):
    ...
    # 1. Pehle Year detect karo (Brackets hatane se pehle taaki (2026) na mite)
    year_match = re.search(r"\b(19\d\d|20\d\d)\b", name)
    extracted_year = year_match.group(1) if year_match else None

    # 2. Season & Episode detect karein (e.g. S01E01, Season 1, S1)
    ...
    # 3. Sirf Year/Season se pehle ka hissa lo (Real Title)
    main_title = name[:cut_pos]

    # 4. Tags, channel brackets aur symbols safai
    main_title = re.sub(r"\[.*?\]|\(.*?\)", " ", main_title)
    main_title = re.sub(r"[\._\-\+:]", " ", main_title)

    # 5. Quality, OTT platform aur junk words safai
    junk_patterns = (
