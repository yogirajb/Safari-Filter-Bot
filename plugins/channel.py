...
async def media(bot, message):
    ...
    # Safe Coroutine execution for save_file
    try:
        res = save_file(media)
        ...
        logging.error(f"Error executing save_file: {e}")
        return
    ...
    if not POST_CHANNELS:
        logging.warning("POST_CHANNELS is not set or empty in environment variables!")
        return
        ...
        async with MERGE_LOCK:
                ...
                if len(final_caption) <= MAX_CAPTION_LENGTH:
                    ...
                    for channel_id in post_data["msg_ids"].keys():
                        ...
                        try:
                            ...
                            logging.info(f"Auto-merged file into existing post ID {msg_id} in channel {channel_id}")
                            ...
                            logging.error(f"Error editing merged post in channel {channel_id}: {e}")
                    return

            # -------------------------------------------------------------
            # CASE 2: नई पोस्ट बनाएं (अगर नई मूवी हो या पिछला कैप्शन फुल हो)
            # -------------------------------------------------------------
            imdb_info = None
            try:
            ...
            for channel in target_channels:
                try:
                    ...
                    logging.info(f"Auto-posted new message ID {msg.id} to channel {channel}")
                except FloodWait as fw:
                    await asyncio.sleep(fw.value)
                    if poster_url:
                        msg = await bot.send_photo(
