...
def clean_movie_title(filename):
    ...
    # 1. Pehle Year & Season extract karein (Brackets hatane se pehle taaki (2017) ya (2026) na mite)
    year_match = re.search(r"\b(19\d\d|20\d\d)\b", name)
    extracted_year = year_match.group(1) if year_match else None

    # Handles S03E09, Season 3, S3, Episode 39, EP09
    se_match = re.search(r"(?i)\b(?:season\s*(\d{1,2})|s(\d{1,2}))(?:\s*(?:episode|ep|e)\s*(\d{1,3}))?\b", name)
    ep_only_match = re.search(r"(?i)\b(?:episode|ep|e)\s*(\d{1,3})\b", name)
    ...
    if se_match:
        ...
        cut_pos = min(cut_pos, se_match.start())
    elif ep_only_match:
        season_tag = "S01"
        cut_pos = min(cut_pos, ep_only_match.start())

    if year_match:
        cut_pos = min(cut_pos, year_match.start())

    # 2. Sirf Year/Season se pehle ka main hissa lo (Clean Title)
    main_title = name[:cut_pos]

    # 3. Ab channel tags, brackets aur symbols clean karo
    main_title = re.sub(r"\[.*?\]|\(.*?\)", " ", main_title)
    main_title = re.sub(r"[\._\-\+:]", " ", main_title)

    # 4. Extra junk words, OTT tags aur quality tags safai
    junk_patterns = (
        r"(?i)\b(1080p|720p|480p|360p|2160p|4k|hdrip|webrip|web-dl|web|bluray|dvdrip|predvd|dvd|camrip|hdcam|"
        r"x264|x265|hevc|10bit|10\s*bit|ds4k|aac\d*|ddp\d*|dd\d*|dts|truehd|h\.264|h\.265|sub|esub|esubs|complete|zip|pack|"
        r"kannada|hindi|english|telugu|tamil|malayalam|marathi|gujarati|punjabi|bengali|"
        r"hq|clean|hd|combined|sample|uncut|uply|archie|mgreborn|mkvcinemas|zee5|amzn|dual|audio|org|p\d+t\d+|"
        r"sonyliv|sony|liv|itunes|dsnp|hotstar|jiocinema|voot|altbalaji|aha|mxplayer|netflix|primevideo|"
        r"hdr10plus|hdr10|hdr|dv|dovi|x\s*pro|pro|v\d+|@\w+|\d+mb|\d+kbps)\b"
    )
...
async def media(bot, message):
    ...
    # 1. File database me hamesha save hogi
    try:
        res = save_file(media)
    ...
    # 2. Check if Auto Post is enabled via info.py / Koyeb
    ...
    try:
        ...
        # Unique Key for Auto-Merging
        current_merge_key = f"{clean_title.lower()} {season_tag.lower()} {extracted_year or ''}".strip()
        ...
        async with MERGE_LOCK:
            ...
            # -------------------------------------------------------------
            # CASE 1: Merge Mode (Same movie/series files merge into existing post)
            # -------------------------------------------------------------
            if matched_key:
            ...
            # -------------------------------------------------------------
            # CASE 2: New Post (First File or New Movie)
            # -------------------------------------------------------------
            imdb_info = None
            try:
                # YEAR AUR FILE NAME DONO PASS HONGE
