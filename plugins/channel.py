...
def clean_movie_title(filename):
    ...
    - Full support for multi-episode ranges with/without brackets (e.g. [E413-E427])
    """
    ...
    # Season & Episode Detection (Fallback if raw check didn't capture)
    se_range_match = re.search(r"(?i)\bS(\d{1,2})\s*(?:E|EP)\s*(\d{1,4})\s*(?:-|to|\s+)\s*(?:E|EP)?\s*(\d{1,4})\b", name)
    range_match = re.search(r"(?i)\b(?:e|ep|episode|episodes)\s*(\d{1,4})\s*(?:-|to|\s+)\s*(?:e|ep)?\s*(\d{1,4})\b", name)
