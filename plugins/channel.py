...
def find_similar_key(new_key):
    ...
    for existing_key in ACTIVE_POSTS.keys():
        ...
        if ratio >= 0.75:  # Better tolerance for slight differences
            return existing_key
    return None
...
async def media(bot, message):
    ...
    try:
        ...
        # Truncate overly long display name if necessary to save caption space
        file_name_display = raw_name.replace('_', ' ')
        if len(file_name_display) > 65:
            file_name_display = file_name_display[:62] + "..."
