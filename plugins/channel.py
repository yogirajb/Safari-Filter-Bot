...
async def media(bot, message):
    ...
    try:
        ...
        # Detect Languages from Filename/Caption
        ...
        detected_languages = ", ".join(unique_langs) if unique_langs else None
        ...
        async with MERGE_LOCK:
            ...
            imdb_info = None
            try:
                # 1. First attempt with Year
                imdb_info = await get_poster(clean_title, year=extracted_year, file=raw_name)
                # 2. Smart Fallback: If year causes 0 results, retry without year
            ...
            if imdb_info:
                ...
                # Fetch language from IMDb if missing from filename
