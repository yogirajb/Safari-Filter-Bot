...
def clean_movie_title(filename):
    ...
    # 1. Tags aur brackets pehle saaf karein
    name = re.sub(r"\[.*?\]|\(.*?\)", " ", name)
    ...
    # 3. Detect Season & Episode (Handles S03E09, Season 3, S3, Episode 39, EP09)
    ...
    # Sirf Season/Year/Episode se pehle ka hissa (Clean Show/Movie Title)
    main_title = name[:cut_pos].strip()
    ...
    junk_patterns = (
        ...
        r"x264|x265|hevc|10bit|10\s*bit|ds4k|aac\d*|ddp\d*|dts|truehd|h\.264|h\.265|sub|esub|esubs|complete|zip|pack|"
