...
def clean_movie_title(filename):
    ...
    - Normalizes dots/underscores to spaces before token parsing
    - Removes Channel Usernames, Branding & Bracket Tags (@Raje Movies, @MZ_Botz etc.)
    - Accurately truncates at Season, Episode, Year or Quality tokens
    - Guaranteed pure Title output for 100% TMDb Match & Auto-merge
    """
    # 1. Remove extension
    ...
    # 4. Pre-normalize dots, underscores, dashes to spaces so word boundaries work perfectly
    ...
    # 6. Universal Season & Episode Detection
    s_match = re.search(r"(?i)\bseason\s*(\d{1,2})\b|\bS(\d{1,2})\b", name)
