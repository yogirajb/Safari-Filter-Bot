...
async def getfile(client, message):
    try:
        ...
        # FIX: TMDb scraper se 'languages' key uthayi hai jo ki plural hai
        language = movie_details.get('languages', None)
...
async def post_to_channels(client, callback_query):
    ...
    if action == "yes":
        ...
        # FIX: Callback query me bhi 'languages' key sahi kar di hai
        language = movie_details.get('languages', None)
        if not language or language == 'N/A':
            language = detect_language_from_filename(file_name)
        if not language:
