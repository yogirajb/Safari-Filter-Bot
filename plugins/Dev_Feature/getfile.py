...
def detect_language_from_filename(filename):
    """File name se language detect karne ke liye backup function"""
    fn_lower = filename.lower()
    
    if "dual" in fn_lower or "dual audio" in fn_lower:
        return "Hindi / English"
    if "multi" in fn_lower or "multi audio" in fn_lower:
        return "Multi Audio (Hindi-Eng-Tam-Tel)"
    
    langs = []
    if "hin" in fn_lower or "hindi" in fn_lower:
        langs.append("Hindi")
    if "eng" in fn_lower or "english" in fn_lower:
        langs.append("English")
    if "tam" in fn_lower or "tamil" in fn_lower:
        langs.append("Tamil")
    if "tel" in fn_lower or "telugu" in fn_lower:
        langs.append("Telugu")
    if "mal" in fn_lower or "malayalam" in fn_lower:
        langs.append("Malayalam")
    if "kan" in fn_lower or "kannada" in fn_lower:
...
async def getfile(client, message):
    try:
        ...
        # Language Logic: Pehle IMDb se check karega, nahi toh file name se extract karega
        language = movie_details.get('language', None)
        ...
        # Final Format text jisme buttons ki jagah hyperlink text hai aur do line ka space hai
        formatted_text = (
            ...
            f"📕 Story: {hindi_plot}\n\n"
            f"🔗 <a href='{custom_link}'>Get File 📁</a>\n\n\n"
            f"⚡️ <a href='{custom_link}'>powered by MzMoviiez</a>"
        )
        ...
        if poster:
            await message.reply_photo(
                ...
                reply_markup=None, # Pehla inline button remove kiya gaya
...
async def post_to_channels(client, callback_query):
    ...
    if action == "yes":
        ...
        # Channel post ke liye same language logic sync kiya gaya hai
        language = movie_details.get('language', None)
        if not language or language == 'N/A':
            language = detect_language_from_filename(file_name)
        if not language:
            ...
            try:
                if poster:
                    await client.send_photo(
                        ...
                        photo=poster,
                        caption=formatted_text,
                        reply_markup=None, # Channel posts se bhi button hataya gaya hai
                        parse_mode=enums.ParseMode.HTML, 
                        has_spoiler=True
                    )
                else:
