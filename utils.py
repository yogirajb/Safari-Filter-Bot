...
async def get_poster(query, bulk=False, id=False, file=None, year=None):
    ...
    Guarantees TV Shows don't match 1968 movies, uses multi-token query cuts,
    and supports backdrop fallbacks for TV serials.
    """
    try:
        media_type = "movie"
        q = (query or "").strip()
        ...
        if movie_id and TMDB_API_KEY:
            ...
            if movie and movie.get("success") is not False:
                ...
                # Agar poster nahi mila toh backdrop banner le lega
                if not poster_path:
        ...
        # -------------------------------------------------------------
        # 2. IMDb (Cinemagoer) Fallback for Indian Serials
        # -------------------------------------------------------------
