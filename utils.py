...
async def get_poster(query, bulk=False, id=False, file=None, year=None):
    ...
    Guarantees TV Shows don't match 1968 movies, and provides safe fallbacks for Indian serials.
    """
    try:
        media_type = "movie"
        q = (query or "").strip()
        ...
        # -------------------------------------------------------------
        # 1. TMDb Detail Retrieval (If Movie/TV found in TMDb)
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # 2. IMDb (Cinemagoer) Fallback (For Indian Serials missing on TMDb)
        # -------------------------------------------------------------
        loop = asyncio.get_running_loop()
        search_results = await loop.run_in_executor(None, imdb.search_movie, clean_q)
