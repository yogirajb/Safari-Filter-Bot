...
async def get_poster(query, bulk=False, id=False, file=None, year=None):
    ...
    TV vs Movie smart priority & Hindi/English Poster filtering ke sath.
    """
    if not TMDB_API_KEY:
        return None
    ...
    try:
        ...
        if not id:
            ...
            # Check if source is a TV Series / Serial / Daily Soap
            ...
            async with aiohttp.ClientSession() as session:
                for q_str in queries_to_try:
                    # 1. Agar TV Series/Serial hai, to sabse pehle direct /search/tv try karega
                    if is_series_file:
                        tv_params = {
                            "api_key": TMDB_API_KEY,
                            "query": q_str,
                        ...
                        async with session.get(f"{TMDB_API_BASE}/search/tv", params=tv_params, timeout=10) as resp:
                            if resp.status == 200:
                    ...
                    # 2. Movie ya fallback ke liye /search/multi
                    params = {
                        "api_key": TMDB_API_KEY,
                        "query": q_str,
                        "include_adult": "false",
                    ...
                    # Agar year match na hua ho aur search_year tha, to bina year ke dobara TV endpoint check karein
                    if not results and search_year and is_series_file:
                        async with session.get(f"{TMDB_API_BASE}/search/tv", params={"api_key": TMDB_API_KEY, "query": q_str}, timeout=10) as resp:
                            if resp.status == 200:
                                data = await resp.json()
                                res_list = data.get("results", [])
                                if res_list:
            ...
            best_match = None
            if is_series_file:
                # Agar series hai toh sirf "tv" media type match karega (1968 movie reject hogi)
                for r in results:
        ...
        # ----- Detailed TMDb Info with Images Filter -----
        params = {
            "api_key": TMDB_API_KEY,
            "append_to_response": "credits,images",
            "include_image_language": "hi,en,null"  # Foreign language poster rokne ke liye
        }
        ...
        # Smart Hindi / English Poster selection
        images = movie.get("images", {})
        posters = images.get("posters", [])
        poster_path = None
        if posters:
            # Hindi poster first, then English, then default
